...
## This script will perform network analysis in CTX samples
...
## Load Adjusted FPKM expression matrix
load("../data/modified/AdjustedExpressionData.Rdata") ##datExpr.CTX.reg and datMeta.CTX

## Choose a set of soft-thresholding powers
powers = c(c(1:10), seq(from = 12, to=30, by=2))
sft <- pickSoftThreshold(t(datExpr.CTX.reg), powerVector = powers, verbose = 5,networkType="signed",corFnc="bicor")

par(mfrow = c(1,2)); # Scale-free topology fit index as a function of the soft-thresholding power
cex1 <- 0.9;
...
softPower <- 8 ## Choose a power of 8 as it's the first on where the R^2 is > 0.8

## Run an automated network analysis
net3 <- blockwiseModules(t(datExpr.CTX.reg),power=softPower,deepSplit=4,minModuleSize=100,
...
moduleColors_CTX.rWGCNA <- read.csv('../data/provided/SuppTable2A.csv') # load rWGCNA CTX module Definitions
gnS <- intersect(rownames(datExpr.CTX.reg),moduleColors_CTX.rWGCNA$ENSEMBL.ID)
moduleColors_CTX.rWGCNA <- moduleColors_CTX.rWGCNA[match(gnS,moduleColors_CTX.rWGCNA$ENSEMBL.ID),]
moduleColors_CTX.rWGCNA_cols <- as.character(moduleColors_CTX.rWGCNA$WGCNA.module.color)
moduleColors <- net3$colors[match(gnS,rownames(datExpr.CTX.reg))]
datExpr.CTX.reg <- datExpr.CTX.reg[match(gnS,rownames(datExpr.CTX.reg)),]

## This function matches labels based on overlap between the network constructed here, and the network provided in the supplement
matchLabel1 <- matchLabels(moduleColors, moduleColors_CTX.rWGCNA_cols) #matchLabels(source,reference)
moduleColors.old <- moduleColors
merge <- mergeCloseModules(t(datExpr.CTX.reg), matchLabel1, cutHeight = 0.2, verbose = 3)
...
MEs <- mergedMEs;
MEs <- orderMEs(MEs)
KMEs<-signedKME(t(datExpr.CTX.reg), MEs,outputColumnName = "kME",corFnc = "bicor")

## Annotate the output
library(biomaRt)
getinfo <- c("ensembl_gene_id","hgnc_symbol","chromosome_name","start_position","end_position","strand","band","gene_biotype")
mart <- useMart(biomart="ENSEMBL_MART_ENSEMBL",dataset="hsapiens_gene_ensembl") ## Can add host="sep2013.archive.ensembl.org" for Gencode v18
geneAnno1 <- getBM(attributes = getinfo,filters=c("ensembl_gene_id"),values=rownames(datExpr.CTX.reg),mart=mart)
geneAnno2 <- geneAnno1[match(rownames(datExpr.CTX.reg),geneAnno1[,1]),]
geneInfo=as.data.frame(cbind(geneAnno2,moduleColors, KMEs))

## Comparison with rWGCNA Modules
geneInfo.rWGCNA <- moduleColors_CTX.rWGCNA
...
colnames(ORmat.ASD) <- colnames(Pmat.ASD) <- colnames(FDRmat.ASD) <- paste('rWGCNA.CTX.M',uniquemodcolors.rWGCNA,sep='')
rownames(ORmat.ASD) <- rownames(Pmat.ASD) <- rownames(FDRmat.ASD) <- paste('CTX_ASD.',uniquemodcolors.ASD,sep='')
